...
module Configs_Title
  ...
  FUNDO_IMG = "Tela de título 01 - Fundo"
  
  LOGO_IMG = "Tela de título 02 - Título"
  LOGO_X = 640/2 - 200
  LOGO_Y = 56
  ...
  OP1_IMG = "Tela de título 03 - Novo Jogo"
  OP1_X = 640/2 - 220
  
  OP2_IMG = "Tela de título 04 - Continuar"
  OP2_X = 640/2 - 65
  
  OP3_IMG = "Tela de título 05 - Como Jogar"
  OP3_X = 640/2 + 90
  
  OP4_IMG = "Tela de título 06 - Sobre"
  OP4_X = 640/2 - 140
  
  OP5_IMG = "Tela de título 07 - Sair"
  OP5_X = 640/2 + 10
  
  # As imagens deve estar na pasta Title
  # Pode adicionar quantas quiser pro "Como Jogar"
  COMOJ_IMGS = ["Como Jogar 01", "Como Jogar 02" ]
  
  # As imagens deve estar na pasta Title
  # Pode adicionar quantas quiser pro "Sobre"
  SOBRE_IMGS = ["Sobre 01", "Sobre 02", "Sobre 03", "Sobre 04" ]
...
class Scene_Title
  ...
  def main
    ...
    @sprite = Plane.new
    @sprite.bitmap = RPG::Cache.title(Configs_Title::FUNDO_IMG)
  ...
  def command_howtoplay
    ...
    img_s = 0
    count = 0
    @sprite_op6.bitmap = RPG::Cache.title(Configs_Title::SOBRE_IMGS[img_s])
    @sprite_op7.bitmap = RPG::Cache.title(Configs_Title::SOBRE_IMGS[img_s])
     loop do
        @sprite_op1.opacity = @sprite_op2.opacity = @sprite_op3.opacity = @sprite_op4.opacity = @sprite_op5.opacity -= 3
        break if @sprite_op3.opacity < 2
        @sprite.ox -= 1
        Graphics.update
     end
     loop do
       ...
       count += 1
         ...
         Graphics.update
         Input.update
           if Input.trigger?(Input::C) && img_s < imgs_comoj && count > 40
             img_s += 1
             count = 0
             if img_s < imgs_comoj
               $game_system.se_play($data_system.decision_se)
             ...
             if Input.trigger?(Input::B) && img_s < imgs_comoj && count > 20
               $game_system.se_play($data_system.cancel_se)
               loop do
                 @sprite_op6.opacity = @sprite_op7.opacity -= 1
                 break if @sprite_op7.opacity < 1 && img_s % 2 == 1
                 break if @sprite_op6.opacity < 1 && img_s % 2 == 0
               end
               break
             end
           end
     ...
     loop do
       if @sprite_op2.opacity < 90
        @sprite_op1.opacity = @sprite_op2.opacity = @sprite_op4.opacity = @sprite_op5.opacity += 3
       end
       @sprite_op3.opacity += 9
        break if @sprite_op3.opacity > 254
        @sprite.ox -= 1
        Graphics.update
     end
  end
  ...
  def command_sobre
    $game_system.se_play($data_system.decision_se)
    imgs_comoj = Configs_Title::SOBRE_IMGS.length
    img_s = 0
    count = 0
    @sprite_op6.bitmap = RPG::Cache.title(Configs_Title::SOBRE_IMGS[img_s])
    @sprite_op7.bitmap = RPG::Cache.title(Configs_Title::SOBRE_IMGS[img_s])
     ...
     loop do
       if @sprite_op2.opacity < 90
        @sprite_op1.opacity = @sprite_op2.opacity = @sprite_op3.opacity = @sprite_op5.opacity += 3
       end
       @sprite_op4.opacity += 9
        break if @sprite_op4.opacity > 254
        @sprite.ox -= 1
        Graphics.update
     end
  end
